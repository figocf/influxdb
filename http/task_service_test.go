...
func TestTaskHandler_Sessions(t *testing.T) {
	...
	t.Run("creating a task from a session", func(t *testing.T) {
		...
		auth, err := i.FindAuthorizationByToken(ctx, createdTasks[0].Token)
		if err != nil {
			t.Fatal(err)
		}

		if auth.OrgID != o.ID {
			t.Fatalf("expected authorization to have org ID %v, got %v", o.ID, auth.OrgID)
		}
		if auth.UserID != u.ID {
			t.Fatalf("expected authorization to have user ID %v, got %v", u.ID, auth.UserID)
		}
